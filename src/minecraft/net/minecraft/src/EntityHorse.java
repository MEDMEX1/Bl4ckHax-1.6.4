...
public class EntityHorse extends EntityAnimal implements IInvBasic
{
    ...
    public static final Attribute horseJumpStrength = (new RangedAttribute("horse.jumpStrength", 0.7D, 0.0D, 2.0D)).func_111117_a("Jump Strength").setShouldWatch(true);
        ...
        else
        {
            if (var2 != null)
            {
                ...
                if (this.func_110259_cr())
                {
                    ...
                    if (var2.itemID == Item.horseArmorIron.itemID)
                    {
                        var4 = 1;
                    }
                    else if (var2.itemID == Item.horseArmorGold.itemID)
                    {
                        var4 = 2;
                    }
                    else if (var2.itemID == Item.horseArmorDiamond.itemID)
                    {
                        var4 = 3;
                    }
                ...
                if (!var3 && !this.func_110256_cu())
                {
                    ...
                    else if (var2.itemID == Block.hay.blockID)
                    {
                        var7 = 20.0F;
                        var5 = 180;
                    }
                ...
                if (!var3 && this.func_110229_cs() && !this.isChested() && var2.itemID == Block.chest.blockID)
                {
                ...
                if (!var3 && this.func_110253_bW() && !this.isHorseSaddled() && var2.itemID == Item.saddle.itemID)
                {
                    this.openGUI(par1EntityPlayer);
                    return true;
                }
    ...
    {
    	if(horsestats) {
        	Double jumpstrength = this.getHorseJumpStrength();
        	Double jumpblocks = -0.1817584952 * (jumpstrength*jumpstrength*jumpstrength) + 3.689713992 * (jumpstrength*jumpstrength) + 2.128599134 * jumpstrength - 0.343930367;
        	Float maxhealth = this.getMaxHealth();
        	String owner = this.getOwnerName();
        	BigDecimal jumpblocks2 = new BigDecimal(jumpblocks).setScale(2, RoundingMode.HALF_UP);
        	Client.addChatMessage("Jump height: "+jumpblocks2+" Max health: "+maxhealth+" Owner: "+owner);
        	horsestats = false;
        	}
